...
import { Ban, Download, Calendar, Check, Zap, Loader2, AlertTriangle, ChevronDown } from 'lucide-react';
...
export default function SecondaryActions({
  ...
  return (
    <div className="mb-4 space-y-3">
      {/* Cancel — visible but ghost */}
      {!isCancelled && (
        <button
          onClick={onShowCancelModal}
          className="flex items-center gap-2 text-sm text-red-500 hover:text-red-700 font-medium px-1 py-2 transition-colors min-h-[44px]"
      ...
      {/* Calendar & Export — folded */}
      <details className="group" data-testid="secondary-actions-details">
        <summary className="flex items-center gap-2 text-sm text-slate-500 cursor-pointer hover:text-slate-700 transition-colors py-2 min-h-[44px] select-none">
