...
- **Bug fix fetch attendance Teams** (March 22): Corrigé 3 bugs — (1) `user_email` → `outlook_email` dans `_resolve_teams_user_id`, (2) Ajouté mode délégué pour fetch attendance (utilise token Outlook `/me/` au lieu de client_credentials), (3) Messages d'erreur clairs pour meetings legacy vs delegated. Auto-fetch exclut maintenant les meetings application_fallback
