...
- **Zoom fallback CSV** (March 23): Gestion d'erreur enrichie pour fetch attendance Zoom. Quand plan gratuit détecté: bandeau ambre "Plan Zoom Pro requis" + CTA "Importer CSV Zoom" + instructions (Zoom > Reports > Meeting > Participants > Export). Import CSV déjà fonctionnel, la UX le surface automatiquement en cas d'échec auto-fetch. Transparent une fois plan Pro activé
